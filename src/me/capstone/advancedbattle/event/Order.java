...
/**
 * Order of an event listener may be registered at. Odd-numbered (IGNORE_CANCELLED) slots are called even when events are marked "not propagating". If an event stops propagating part way
...
public enum Order {
	...
	LATEST(8, false),
	/**
	 * Called after "Latest" handlers. No changes to the event should be made in this order slot (though it is not enforced). This is called even when event has been cancelled.
	 */
	MONITOR(9, true);
